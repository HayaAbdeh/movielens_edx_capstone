...
This is a four parts report, which had present the subject and the problem in its first part (the introduction). Also, will presents the ideas of the solutions in its second part (Summary), the third part reviews the algorithms’ model and the plots in the (Methods and modeling), the final part will show the results and conclusions.
...
```{r print result table, echo=TRUE}
...
#as a result we find that the minimum RMSE is 0.8644 which we optained from regularized movie and user and genres parameter model
...
## and as a future work,, more machine learning models going to work on trying to find if we can get a value of RMSE less than what we tried in the previews models.
